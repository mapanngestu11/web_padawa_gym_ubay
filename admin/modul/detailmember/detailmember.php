...
echo"</table>";*/
		...
		<table id="datatables" class="display">
			...
			while ($r = mysqli_fetch_array($sql)) {
				...
				$tampilcek = mysqli_fetch_array(mysqli_query($conn, "SELECT b.tipe,a.kuota_latihan, a.id_member,TIMESTAMPDIFF(DAY,current_date,a.tanggal_berlaku) as wew from tbl_deposit as a LEFT JOIN tbl_tarif as b ON b.kode_tarif=a.kode_tarif where a.id_member = '$_GET[id]' and TIMESTAMPDIFF(DAY,current_date,a.tanggal_berlaku) > 0"));

				$cek_data = mysqli_query($conn, "SELECT b.tipe,a.kuota_latihan, a.id_member,TIMESTAMPDIFF(DAY,current_date,a.tanggal_berlaku) as wew from tbl_deposit as a LEFT JOIN tbl_tarif as b ON b.kode_tarif=a.kode_tarif where a.id_member = '$_GET[id]' and TIMESTAMPDIFF(DAY,current_date,a.tanggal_berlaku) > 0");

				if (empty($tampilcek['id_member']) || empty($tampilcek['wew'])) {
					$stts = 'EXPIRED';
				} else {
					if ($tampilcek['tipe'] == 'UNLIMITED') {
						if ($tglsekarang1 >= $tampilcek['tanggal_deposit'] && $tglsekarang1 <= $tampilcek['tanggal_berlaku']) {
							$stts = 'NONAKTIF';
						} else {
							$stts = 'AKTIF';
						}
				...
				if ($stts == 'EXPIRED') {
					echo "<td><center><a href='?modul=tambahdeposit&aksi=tambahdeposit&id=" . $_GET['id'] . "&kd=" . $r['kode_tarif'] . "'>Tambah/Aktivasi</a></center></td>";
				} else if ($stts == 'NONAKTIF') {
					echo "<td><center><a href='?modul=tambahdeposit&aksi=tambahdeposit&id=" . $_GET['id'] . "&kd=" . $r['kode_tarif'] . "'>Tambah/Aktivasi</a></center></td>";
				} else {
					echo "<td><center><a href='#' onclick='alert(\"Tidak Bisa Tambah Aktivasi Karena Masih Ada Paket Yang Aktif !\")'>Tambah/Aktivasi</a></center></td>";
				}
			...
			while ($tampil0 = mysqli_fetch_array($query0)) {
				...
				echo "<tr><td>Nama</td><td>$tampil0[username]</td></tr>";
