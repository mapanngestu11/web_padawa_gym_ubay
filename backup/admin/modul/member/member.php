	...
	<td align=center><i>Untuk Memperpanjang / Menambah Paket yang ada klik 'Detail & Paket'</i></td>
        ...
        while ($r = mysqli_fetch_array($sql)) {
			...
			";
                ...
                if ($r['id_member'] == "MBR-UMUM") {
                    ...
                    if (empty($tampilcek['id_member']) || empty($tampilcek['wew'])) {
                        ...
                        $status = '<div style="color:red; font-weight:bold"><center>EXPIRED</center></div>';
                    } else {

                        $status = '<div style="color:GREEN; font-weight:bold"><center>AKTIF</center></div>';
                    }
                }
			...
			";
                ...
                $tampilcek = mysqli_fetch_array(mysqli_query($conn, "SELECT b.tipe,a.kuota_latihan, a.id_member,TIMESTAMPDIFF(DAY,current_date,a.tanggal_berlaku) as wew from tbl_deposit as a LEFT JOIN tbl_tarif as b ON b.kode_tarif=a.kode_tarif where a.id_member = '$r[id_member]' and TIMESTAMPDIFF(DAY,current_date,a.tanggal_berlaku) > 0"));

                $cek_data = mysqli_query($conn, "SELECT b.tipe,a.kuota_latihan, a.id_member,TIMESTAMPDIFF(DAY,current_date,a.tanggal_berlaku) as wew from tbl_deposit as a LEFT JOIN tbl_tarif as b ON b.kode_tarif=a.kode_tarif where a.id_member = '$r[id_member]' and TIMESTAMPDIFF(DAY,current_date,a.tanggal_berlaku) > 0");

                while ($r2 = mysqli_fetch_array($cek_data)) {

                    // var_dump($r2);
                }
                // die;
                // var_dump($tampilcek['kuota_latihan']);
                ...
                if ($r['id_member'] == "MBR-UMUM") {
                    ...
                    if (empty($tampilcek['id_member']) || empty($tampilcek['wew'])) {
                        ...
                        if ($tampilcek['tipe'] != 'UNLIMITED' && $tampilcek['kuota_latihan'] == '0') {
        ...
        <div id="tickets">
            <ul>
                ...
                <li>
                    ...
                    <input type="date" name="tanggal_daftar" value="<?php echo $tglsekarang; ?>" class="form-control" placeholder="Tanggal Daftar" required validationMessage="Mohon Masukkan Tanggal Daftar" /></br><span style="padding-left:130px"></span>
                </li>
                <li>
